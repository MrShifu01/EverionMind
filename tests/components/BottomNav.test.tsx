...
describe("BottomNav", () => {
  it("renders 5 primary navigation items", () => {
    renderWithTheme(<BottomNav activeView="feed" onNavigate={vi.fn()} onCapture={vi.fn()} />);
    const nav = screen.getByRole("navigation", { name: /primary navigation/i });
    const buttons = nav.querySelectorAll("button");
    expect(buttons.length).toBe(5);
  });

  it("contains Feed, New entry, Ask, Memory, and Settings items", () => {
    renderWithTheme(<BottomNav activeView="feed" onNavigate={vi.fn()} onCapture={vi.fn()} />);
  ...
  it("all touch targets are at least 44px (h-11 or larger)", () => {
    const { container } = renderWithTheme(<BottomNav activeView="feed" onNavigate={vi.fn()} onCapture={vi.fn()} />);
  ...
  it("non-active items do not have aria-current", () => {
    ...
    expect(feedBtn).not.toHaveAttribute("aria-current", "page");
  });

  it("calls onNavigate with the correct view id on click", async () => {
    const user = userEvent.setup();
    const onNavigate = vi.fn();
    renderWithTheme(<BottomNav activeView="feed" onNavigate={onNavigate} onCapture={vi.fn()} />);
