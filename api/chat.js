...
export default async function handler(req, res) {
  ...
  const embedProvider = (req.headers["x-embed-provider"] || "openai").toLowerCase();
  ...
  let queryEmbedding;
  try {
    queryEmbedding = await generateEmbedding(message.trim(), embedProvider, embedKey);
  } catch (e) {
  ...
  const retrievedEntries = rpcRes.ok ? await rpcRes.json() : [];
  const sourceIds = retrievedEntries.map(e => e.id);
