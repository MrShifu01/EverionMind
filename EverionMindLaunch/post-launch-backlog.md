...
- [ ] **ChatSheet close → reverse morph back to Ask pill.** Less urgent now that the chat is route-based (2c233df) — closing chat = navigating back, which already goes through the root-view native transition. The morph would only matter if ChatSheet remains as a fallback surface; cleanup of the legacy modal is queued below.
- [ ] **Retire the legacy ChatSheet modal.** With the chat route conversion live, ChatSheet's open path is unreachable in the default flow (MobileHome's `onNavigate` always wins). Remove the unused ChatSheet rendering from MobileHome + the prop fallback once you're confident the route-based flow holds up.
- [ ] **BrainSwitcher pill → BrainSwitcher sheet.** The current BrainSwitcher uses a Radix DropdownMenu — a small popover that sits next to the trigger. A shared-element morph between trigger and dropdown item is barely perceivable at this scale. Bigger UX win = convert to Dialog/Sheet (the design implied a larger sheet anyway), then apply matching `view-transition-name` to the pill and to the active-brain row.
