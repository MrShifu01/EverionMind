...
function MobileMoreMenuInner({ isOpen, onNavigate }: MobileMoreMenuProps) {
  return (
    <div className={cn("fixed inset-0 z-40 lg:hidden", !isOpen && "pointer-events-none")}>
      ...
      <aside
        className={cn(
          "absolute right-0 top-0 h-full w-72 flex flex-col transform transition-transform duration-300 ease-in-out",
      ...
      >
        ...
        >
          <span className="font-semibold text-base" style={{ color: "var(--color-on-surface)" }}>
            Menu
          </span>
          <button
            onClick={() => onNavigate("close")}
            aria-label="Close menu"
            className="w-9 h-9 flex items-center justify-center rounded-xl transition-colors press-scale"
        ...
        {/* Nav items */}
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto" aria-label="More navigation">
          {SIDEBAR_ITEMS.map((item) => (
            <button
              key={item.id}
              onClick={() => onNavigate(item.id)}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all duration-150 press-scale"
              style={{ color: "var(--color-on-surface-variant)" }}
            >
              {item.icon}
