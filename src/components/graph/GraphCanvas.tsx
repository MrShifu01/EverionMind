...
export default function GraphCanvas({ nodes, edges, selected, onSelect, onMoveNode }: Props) {
  ...
  useEffect(() => { fittedRef.current = false; }, [nodes.length]);
  ...
  const nodeDragRef = useRef<{
    ...
    nodeStartX: number; nodeStartY: number; moved: boolean;
  } | null>(null);
    ...
    if (isNode) {
      ...
      if (n) {
        nodeDragRef.current = {
          ...
          nodeStartX: n.x, nodeStartY: n.y, moved: false,
        };
      }
    } else {
      dragRef.current = {
    ...
    <div style={{ flex: 1, position: "relative", overflow: "hidden", touchAction: "none" }}>
      ...
      <svg
        ...
        style={{
          ...
          display: "block", background: "var(--surface-dim)",
            ...
            {nodes.map(node => {
              ...
              return (
                ...
                >
                  ...
                  {isHov && !isSel && (
                    <circle r={r + 5} fill="none" stroke={color} strokeOpacity={0.45} strokeWidth={1} />
                  )}
                  <circle
        ...
        {([
          ...
          <button key={label} title={title}
            ...
            style={{
              ...
              fontFamily: "var(--f-sans)", boxShadow: "var(--lift-2)",
            }}
          >
            {label}
          </button>
